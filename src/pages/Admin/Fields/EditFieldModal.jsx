...
import { updateAdminField } from "../../../services/adminService";
import { toast } from "react-toastify";

export default function EditFieldModal({ isOpen, onClose, field, onFieldUpdated }) {
  ...
  useEffect(() => {
    if (field) {
      setFormData({
        name: field.name || "",
        adress: field.adress || "",
        price_per_hour: field.price_per_hour || field.price || "",
        pelouse: field.pelouse || "Synthétique",
        capacity: field.capacity || "10",
        description: field.description || "",
        status: field.status || "active"
      });
      setImageFile(null);
      setImagePreview(field.field_images?.[0]?.url_image || null);
    }
  }, [field, isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await updateAdminField(field.id, formData, imageFile);
      toast.success("Terrain mis à jour avec succès !");
      onFieldUpdated();
      onClose();
    } catch (error) {
      console.error(error);
      toast.error("Erreur lors de la modification du terrain : " + (error.message || ""));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#2c241b] border border-[#493622] rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-[#2c241b] border-b border-[#493622] p-6 flex justify-between items-center z-10">
          <div>
            <h2 className="text-xl font-bold text-white">Modifier le Terrain</h2>
            <p className="text-sm text-[#cbad90] mt-1">Modifiez les caractéristiques et les détails opérationnels du terrain.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1 text-sm text-gray-400">
                Vous pouvez téléverser un fichier image pour ajouter une photo ou écraser l'image principale actuelle.
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-[#cbad90] text-sm font-medium mb-2">Tarif par heure (en CFA) *</label>
              <input
                type="number"
                name="price_per_hour"
            ...
            <div>
              ...
              <select
                ...
                onChange={handleInputChange}
                className="w-full px-4 py-2.5 bg-[#231a10] border border-[#493622] rounded-xl text-white focus:outline-none focus:border-primary cursor-pointer transition-colors"
              >
            ...
            <div>
              <label className="block text-[#cbad90] text-sm font-medium mb-2">Statut opérationnel</label>
              <select
                name="status"
          ...
          <div>
            <label className="block text-[#cbad90] text-sm font-medium mb-2">Description complète</label>
            <textarea
              name="description"
              ...
              placeholder="Décrivez les atouts de ce terrain..."
            />
          </div>
