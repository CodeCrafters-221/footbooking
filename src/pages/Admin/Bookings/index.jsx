...
export default function AdminBookings() {
	...
	const fetchBookings = async () => {
		setLoading(true);
		try {
			const { data, count } = await getAdminBookings(
				page,
				limit,
				filter,
			);
	...
	const handleConfirmAction = async () => {
		...
		const newStatus =
			action === "validate" ? "Confirmé" : "Annulé";
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<div>
					...
					<p className="text-[#cbad90] mt-1">
						Toutes les réservations de la plateforme. Total:{" "}
						{totalCount}
					</p>
				</div>
			</div>
							...
							{loading ? (
								Array.from({ length: 5 }).map((_, i) => (
									<tr key={i} className="animate-pulse">
										<td className="p-4">
											<div className="h-6 w-32 bg-[#493622] rounded" />
										</td>
										<td className="p-4">
											<div className="h-6 w-24 bg-[#493622] rounded" />
										</td>
										<td className="p-4">
											<div className="h-6 w-32 bg-[#493622] rounded" />
										</td>
										<td className="p-4">
											<div className="h-6 w-16 bg-[#493622] rounded" />
										</td>
										<td className="p-4">
											<div className="h-6 w-20 bg-[#493622] rounded-full" />
										</td>
										<td className="p-4">
											<div className="h-8 w-28 bg-[#493622] rounded-lg ml-auto" />
										</td>
									</tr>
								))
							) : bookings.length === 0 ? (
								<tr>
									<td
										colSpan={6}
										className="p-12 text-center text-[#cbad90]"
									>
										...
										<p className="text-xs mt-1">
											Modifiez le filtre pour voir plus de
											résultats.
										</p>
									</td>
								</tr>
								...
								bookings.map((b) => (
									<tr
										key={b.id}
										className="hover:bg-[#493622]/30 transition-colors"
									>
										<td className="p-4">
										...
										<td className="p-4">
											...
											{b.client_phone && (
												<p className="text-[#cbad90] text-xs mt-1 ml-6">
													{b.client_phone}
												</p>
											)}
										</td>
										...
										<td className="p-4">
											<div className="flex items-center justify-end gap-2">
												<button
													...
													onClick={() =>
														navigate(
															`/admin/reservations/${b.id}`,
														)
													}
													className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20"
													title="Voir détails"
												>
