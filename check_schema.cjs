...
const envPath = path.join(__dirname, 'check_schema.cjs'); // We'll resolve env relative to current folder
const rootDir = path.dirname(envPath);
...
const logFile = path.join(rootDir, 'schema_output.txt');
...
async function run() {
  try {
    ...
    if (psError) log('Error platform_settings:', psError.message);
    ...
    log('--- reports ---');
    if (repError) log('Error reports:', repError.message);
    ...
    log('--- report_comments ---');
    if (rcError) log('Error report_comments:', rcError.message);
    ...
    log('--- notifications ---');
    if (notError) log('Error notifications:', notError.message);
